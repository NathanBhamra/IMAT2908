...
static void key_callback(GLFWwindow* window, int key, int cancode, int action, int mods)
{
	...
	{
		scene->lightChange(0.2); // increment the light by + 0.2
		scene->setLightParams(camera); // setting the camera to the scene  
	}
	
	if (key == GLFW_KEY_DOWN && action == GLFW_RELEASE)
	{
		scene->lightChange(-0.2); // decrement the light by - 0.2
		scene->setLightParams(camera); // setting the camera to the scene  
	}
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	string name ="Lab Book 3 : Phong TeaPot";
